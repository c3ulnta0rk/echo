...
async fn do_rebind_fallback<'a>(
    ...
    info!("[Wayland] Rebind fallback: updating dconf and recreating session");

    // Update dconf with new triggers BEFORE rebinding.
    // Without this, GNOME ignores our preferred_trigger and returns the
    // cached value from the first authorization.
    if let Err(e) = update_dconf_shortcuts(app) {
        warn!("[Wayland] Failed to update dconf shortcuts (continuing with rebind): {}", e);
    }
